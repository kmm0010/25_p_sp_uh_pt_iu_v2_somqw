...
directory <- '/Volumes/SSD Part2/_userDataSSD2/Experiments/kmDissKM/ibexFarm/24_p_sp_uh_pt_iu_qpqw/results'
...
for (s in subjs) {
	...
	plot(s.result$RTadj, main=s, ylim=c(-300, 4700), typ='l', col='darkblue')
	abline(0, 0)
}
...
barplot(percentNoResp, names.arg=subjs, ylim=c(0, 10), las=2, space=0.2, legend.text=FALSE)
...
20% of your response times were before the onset of the acoustic information for 'p' and 't'. And, 77% of your responses were just 200 milliseconds after this onset. The human brain generally needs about 200 milliseconds to just make a decision, and it takes an even longer time to physically click a mouse button (and the internet interface should be make this slower). So, it appears that you were just pressing buttons fast in order to get through much of the experiment rather than attending to the stimuli.
...
## for individual
s <- 'A21QMMJH0SG779'  # (too fast, rejected)
s <- 'AOTCPS24JM5DQ'  # (too fast, rejected)
...
problempeople <- c('A21QMMJH0SG779', 'AOTCPS24JM5DQ')
...
for (s in subjCorrect) {
	...
	iutot <- sum(iursp[ , c('6', '7', '8')])
	ursp <- sum(iursp['0', c('6', '7', '8')])
	irsp <- sum(iursp['1', c('6', '7', '8')])
...
vResp.stepC <- lmer(respNum ~ stepCc + contextc + (1 | workerID), family = "binomial", data = responsesCorrect )
summary(vResp.stepC)


Generalized linear mixed model fit by maximum likelihood ['glmerMod']
 Family: binomial ( logit )
Formula: respNum ~ stepCc + contextc + (1 | workerID) 
...
vResp.stepC.slopes <- lmer(respNum ~ stepCc + contextc + (1 + stepCc + contextc | workerID), family = "binomial", data = responsesCorrect )	
summary(vResp.stepC.slopes)

Generalized linear mixed model fit by maximum likelihood ['glmerMod']
 Family: binomial ( logit )
Formula: respNum ~ stepCc + contextc + (1 + stepCc + contextc | workerID) 
   Data: responsesCorrect 
...
Data: responsesCorrect
Models:
vResp.stepC: respNum ~ stepCc + contextc + (1 | workerID)
vResp.stepC.slopes: respNum ~ stepCc + contextc + (1 + stepCc + contextc | workerID)
...
vResp.X.stepC.slopes <- lmer(respNum ~ stepCc * contextc + (1 + stepCc * contextc | workerID), family = "binomial", data = responsesCorrect )	
...
vResp.stepC.notcent <- lmer(respNum ~ stepC + context + (1 | workerID), family = "binomial", data = responsesCorrect )
summary(vResp.stepC.notcent)

vResp.stepC.slopes.notcent <- lmer(respNum ~ stepC + context + (1 + stepC + context | workerID), family = "binomial", data = responsesCorrect )	
